...
char* indexes(const char* path)
{
    ...
    sprintf (buffer, "%s%s", path, "index.php");
    if (access(buffer, 0) != -1) {
        return buffer;
    }
    sprintf (buffer, "%s%s", path, "index.html");
    if (access(buffer, 0) != -1) {
        return buffer;
    }
    return NULL;
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    int c;
    int copacity = 0;
    BYTE* buffer = malloc(copacity * sizeof(BYTE));
    int n = 0;
    ...
    while ((c = fgetc(file)) != EOF) {
        ...
        if (copacity < n + 1) {
            ...
            buffer = realloc(buffer, copacity * sizeof(BYTE));
        }
...
const char* lookup(const char* path)
{
    char* ext = strrchr(path, '.'); // uze est ukazatelem do konca ctroki do /0
    
    for (int i = 0; i < MIMETYPE_LENTH; i++) {
        if (strcasecmp(MIMETYPE[i][1], ext) == 0) {
           printf("%s", MIMETYPE[i][0]);
           return MIMETYPE[i][0];
        }
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* request = strtok(NULL, " "); // vnutri zapominajet poloj - NULL -- _ udaliaet
    char* httpVer = strtok(NULL, "\r");
    
    //printf("method = [%s]\t request = [%s]\t httpVer = [%s]\t\n", method,request,httpVer);
    
    if (strcmp(method,"GET") || (method[4] == ' ')) {
       // free(tmp);
        error(405);
        return false;
    }
    ...
    if (q == NULL) {
        query[0] = '\0'; // analog s *query -- obr k 1 simvolu
    }
    else {
        sprintf(query, "%s", q);
    }
